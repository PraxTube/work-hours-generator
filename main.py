...
def print_info_hours(info):
    ...
    print_msg("Total sum of workdays", info["actual_workdays"], info["expected_workdays"])
    print_msg("Max hours worked", info["actual_max_hours"], info["expected_max_hours"])
    print_msg("Min hours worked (non-zero)", info["actual_min_hours"], info["expected_min_hours"])
...
def print_info_xlsx(info):
    ...
    for i in range(len(months)):
        ...
        sign = "-" if diff < 0 else "+"
        print("{}: {}, {} {}".format(months[i], info["monthly_hours"][i], sign, abs(diff)))
